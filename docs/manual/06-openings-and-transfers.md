...
List with document counts: [21-positions-list.json](examples/21-positions-list.json).
...
The same record as prepared is [11-opening-draft.json](examples/11-opening-draft.json); after
review it reads ([12-opening-confirmed.json](examples/12-opening-confirmed.json)):
...
- **Nothing before the cut-off.** Once an opening is confirmed, any receipt, payment or
  ...
  ([45-refused-requests.json](examples/45-refused-requests.json), third entry):
...
Confirmed example ([20-transfer-confirmed.json](examples/20-transfer-confirmed.json)):
...
([37a-report-cash-account.json](examples/37a-report-cash-account.json)) — while the
